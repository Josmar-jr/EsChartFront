...
export function Sidebar() {
  const [isCompressSidebar, setIsCompressSidebar] = useState(true);

  function handleOpenDisclosure() {
    setIsCompressSidebar(!isCompressSidebar);
  }

  return (
    <aside
      className={`border ease-linear duration-300 border-l relative py-6 items-start border-slate-300 flex flex-col h-screen ${
        isCompressSidebar ? 'w-12' : 'w-48'
      }`}
      role="menubar"
    >
      ...
      <button
        ...
        className={`w-8 h-8 hover:brightness-95 ease-linear outline-none focus:brightness-95 focus:ring focus:secondary duration-300 absolute bg-neutral top-16 left-44 ${
          isCompressSidebar && 'translate-x-[-144px]'
        } ${
          !isCompressSidebar && ' rotate-180'
        } drop-shadow-md rounded-full flex items-center justify-center`}
      >
        <CaretDoubleRight size={16} weight="bold" />
      </button>

      <NavSection isCompressSidebar={isCompressSidebar} />
    </aside>
  );
}
