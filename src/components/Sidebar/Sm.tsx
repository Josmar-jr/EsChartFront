import { ChartPieSlice, Gear, SquaresFour, X } from 'phosphor-react';
...
export function Sm({ isOpen, onClose }: SmProps) {
  return (
    <aside
      className={`${
        !isOpen && 'opacity-0 pointer-events-none -translate-y-28'
      } z-50 w-full bg-neutral dark:bg-slate-900 h-screen fixed top-0 left-0 transition-all`}
    >
      <nav className="flex h-full flex-col items-center relative justify-center font-medium">
        <button
          onClick={onClose}
          className="absolute top-4 left-4 text-primary dark:text-secondary outline-none rounded-sm hover:brightness-95 focus:brightness-95 focus:ring"
        >
          <X size={26} weight="bold" />
        </button>
        <div className="mx-auto absolute top-4 flex items-center justify-center">
          <svg
            width="37"
            height="27"
            viewBox="0 0 37 27"
